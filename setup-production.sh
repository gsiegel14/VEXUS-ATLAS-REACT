...
echo "Authenticated as: thevexusatlas@gmail.com"
echo -e "${NC}"
...
if [[ -z "$CURRENT_ACCOUNT" ]]; then
    ...
    echo "   gcloud auth login --account=thevexusatlas@gmail.com"
    exit 1
fi

echo -e "${GREEN}✅ Prerequisites check passed (authenticated as: $CURRENT_ACCOUNT)${NC}"
...
# Enable required APIs (skip if already enabled)
echo -e "${YELLOW}📡 Verifying required Google Cloud APIs are enabled...${NC}"
apis=(
