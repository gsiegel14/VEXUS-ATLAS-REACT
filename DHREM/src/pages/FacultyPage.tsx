...
import { currentFaculty, currentFellows, pastFellows, FacultyMember } from '../data/facultyData';
...
const FacultyPage: React.FC = () => {
  ...
  const pastFellowsByYear = useMemo(() => {
    const groups = pastFellows.reduce<Record<string, FacultyMember[]>>((acc, member) => {
      const yearRangeMatch = member.fellowshipYears
        || member.fellowship?.match(/(\d{4}\s*[-–]\s*\d{4})/)?.[1];
      const singleYearMatch = member.fellowship?.match(/(\d{4})/);
      const label = (yearRangeMatch ? yearRangeMatch.replace(/\s+/g, '') : singleYearMatch?.[1]) || 'Previous Years';
    ...
    return Object.entries(groups).sort(([labelA], [labelB]) => {
      const sortA = getSortValue(labelA);
      const sortB = getSortValue(labelB);

      if (sortA === sortB) {
        if (labelA === 'Previous Years') {
          return 1;
        }
        if (labelB === 'Previous Years') {
          return -1;
        }
    ...
    <Box>
      ...
      >
        <Container maxWidth="lg">
          ...
          >
            Meet our current faculty, current fellows, and past fellows dedicated to advancing emergency ultrasound
          </Typography>
        </Container>
      </Box>
      ...
      <Container maxWidth="lg" sx={{ py: 6 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
          ...
          >
            <Tab label={`Current Faculty (${currentFaculty.length})`} />
            <Tab label={`Current Fellows (${currentFellows.length})`} />
            <Tab label={`Past Fellows (${pastFellows.length})`} />
          </Tabs>
        </Box>
        ...
        <TabPanel value={tabValue} index={2}>
          {pastFellowsByYear.map(([yearLabel, members]) => (
            <Box key={yearLabel} sx={{ mb: 6 }}>
              <Typography
                variant="h4"
                sx={{
                  fontWeight: 700,
