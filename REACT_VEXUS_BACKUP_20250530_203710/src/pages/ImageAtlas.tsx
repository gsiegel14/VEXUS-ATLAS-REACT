...
// API base URL - using the backend server
  ...
  const filteredImages = React.useMemo(() => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value) {
        switch (key) {
          case 'quality':
            result = result.filter(img => img.quality === value);
            break;
          case 'veinType':
            result = result.filter(img => img.veinType === value);
            break;
          case 'waveform':
            result = result.filter(img => img.waveform === value);
            break;
          case 'subtype':
            result = result.filter(img => img.subtype === value);
            break;
          case 'qa':
            if (value === 'Present') {
